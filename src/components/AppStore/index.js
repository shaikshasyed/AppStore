...
class AppStore extends Component {
  ...
  render() {
    ...
    const resultedAppList = appsList.filter(
      eachApp => eachApp.category === tabId,
    )
    const searchableList = resultedAppList.filter(
      eachApp => eachApp.appName.toLowerCase().includes(searchInput.toLowerCase())
    )
    return (
      <div className="App-Store-container">
        <div className="app-store-upper-part">
          <h1 className="heading">App Store</h1>
          <div className="input-container">
            <input type="search" className="inputEle" placeholder="search" onChange={this.changesearchInput}/>
            <img
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                tabDetails={eachTab}
                key={eachTab.tabId}
                getResultedApps={this.getResultedApps}
                isActive={eachTab.tabId === tabId}
              />
            ))}
          </ul>
        </div>
        <div>
          <ul className="App-Container">
            {searchableList.map(eachAppItem => (
